...
export function FacilitiesPendingProvider({ children }: { children: ReactNode }) {
...
export function useFacilitiesPending() {
  return useContext(PendingContext);
}

export function useSetFacilitiesPending(isPending: boolean) {
...
export function FacilitiesListOverlay({ children }: { children: ReactNode }) {
  const isPending = useFacilitiesPending();
  return (
    <div className="relative flex min-h-0 flex-1 flex-col">
      ...
      <div className={`flex min-h-0 flex-1 flex-col ${isPending ? "pointer-events-none select-none opacity-40 transition-opacity" : "transition-opacity"}`}>
        {children}
      </div>
    </div>
  );
