  ...
  createPrelive: protectedProcedure
    .input(
      z.object({
        ...
        boardMeetingDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),
        credentialingDueDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),
  ...
  createWorkflowPhase: protectedProcedure
    .input(
      z.object({
        ...
        startDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).min(1, "Start date is required"),
        dueDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).min(1, "Due date is required"),
