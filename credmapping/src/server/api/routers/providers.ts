  ...
  createPrivilege: protectedProcedure
    .input(
      z.object({
        ...
        currentPrivInitDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),
        currentPrivEndDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),
        termDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),
